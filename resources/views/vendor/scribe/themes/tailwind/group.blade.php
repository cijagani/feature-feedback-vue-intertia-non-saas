...
<div class="mb-8 p-6 bg-gradient-to-r from-primary-50 to-primary-100 dark:from-primary-900/20 dark:to-primary-800/20 rounded-xl border border-primary-200 dark:border-primary-700">
    <h2 id="{{ Str::slug($group['name']) }}" class="text-2xl font-bold text-gray-900 dark:text-white mb-3 flex items-center">
...
@foreach($group['subgroups'] as $subgroupName => $subgroup)
    @if($subgroupName !== "")
        <h3 id="{{ Str::slug($group['name']) }}-{{ Str::slug($subgroupName) }}" class="text-xl font-semibold text-gray-900 dark:text-white mt-8 mb-4">
            {{ $subgroupName }}
        </h3>
    ...
    @foreach($subgroup as $endpoint)
        @include("scribe::themes.tailwind.endpoint")
    @endforeach
@endforeach
